...
public class WorldGenUtil {
    ...
    public static final int PREVENT_CASCADE_FLAGS = CommonDef.BlockFlags.TO_CLIENT | CommonDef.BlockFlags.IGNORE_OB;
    public static final int PREVENT_CASCADE_FLAGS_2 = PREVENT_CASCADE_FLAGS | CommonDef.BlockFlags.CLIENT_DONT_RENDER;
    ...
    public static boolean generate(ResourceLocation res, World worldIn, BlockPos position, @Nullable PlacementSettings placementsettings, float integrity, boolean useMinorOffset, boolean useRandomRotation) {
        ...
        if (template == null) {
            Main.LogWarning("Trying to load Template that does not exist: %s", res);
            stopUpdate = false;
            return false;
        }
    ...
    public static void setBlockState(Chunk chunk, int relX, int y, int relZ, IBlockState state)
    {
        ...
        {
            Main.LogWarning("#2 Failed to generate %s@(%d,%d,%d) as it's too high", state, relX, y, relZ);
            return;
        }
        ...
        {
            Main.LogWarning("#3 Failed to generate:(%d,%d,%d)", relX, y, relZ);
        }
    }
    ...
    public static void setBlockStateIfAir(Chunk chunk, int relX, int y, int relZ, IBlockState state)
    {
        if (y > MAX_BUILD_HEIGHT)
        {
            Main.LogWarning("#1 Failed to generate:(%d,%d,%d) as it's too high",relX, y, relZ);
            return;
        }
...
//    }
    ...
    public static boolean withinWorldSpawnForbidZone(int chunkX, int chunkZ, World world) {
        BlockPos spawn = world.getSpawnPoint();
        int spawnX = spawn.getX() >> 4;
        int spawnZ = spawn.getZ() >> 4;
        int limit = ModConfig.WORLD_CONF.SPAWN_CLEAR_DISTANCE;
        if (Math.abs(spawnX - chunkX) <= limit || Math.abs(spawnZ - chunkZ) <= limit)
        {
            return true;
        }
        return false;
