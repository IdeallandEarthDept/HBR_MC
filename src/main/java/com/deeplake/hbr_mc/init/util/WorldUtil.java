...
public class WorldUtil {
    ...
    public static NBTTagCompound getDimNbtIDL(World world)
    {
        ...
        //ExampleMod.Log("nbt = ", worldNBT.toString());
        if (worldNBT != null)
        {
            NBTTagCompound idlNBT = worldNBT.getCompoundTag(Main.MODID);
            if (idlNBT == null)
            {
                idlNBT = new NBTTagCompound();
                worldNBT.setTag(Main.MODID, idlNBT);
    ...
    public static boolean isNight(World world) {
        int tickInDay = (int) (world.getWorldTime() % 24000);
