...
public class IDLNBTUtil {
	...
	public static EntityPlayer getMarkedPlayerByItsName(World world,ItemStack stack)
	{
		NBTTagCompound NBT = IDLNBTUtil.getNBTReadOnly(stack);

		return world.getPlayerEntityByName(NBT.getString(PLAYER_NAME));
	}

	public static void markPlayerToStack(ItemStack stack, EntityPlayer player)
	{
		IDLNBTUtil.SetBoolean(stack, PLAYER_READY, true);
		IDLNBTUtil.SetString(stack, PLAYER_NAME, player.getName());
	}
